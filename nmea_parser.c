 ...
 *
 *
 * @attention this file fix to support muti gnss msg
 */
...
#define NMEA_EVENT_LOOP_QUEUE_SIZE (32)
...
typedef struct
{
    ...
    uint8_t sat_num[CONFIG_NMEA_STATEMENT_GSX_DECODE_SYSTEM_NUM];   /*!< Satellite number , get from gsv */
    uint8_t sat_count[CONFIG_NMEA_STATEMENT_GSX_DECODE_SYSTEM_NUM]; /*!< Satellite count , get from gsv */
...
#if (CONFIG_NMEA_STATEMENT_GSA || CONFIG_NMEA_STATEMENT_GSV)
    nmea_satellite_system_t prase_gnss_system; /*!< prase gnss system */
    uint32_t all_statements_gsx;               /*!< All satellite mask */
#endif

#if (CONFIG_NMEA_STATEMENT_GSA)

    uint8_t cur_statement_gsa; /*!< Current satellite ID */
    uint8_t parsed_gsa;        /*!< OR'd of satellite system that have been parsed */

#endif

#if (CONFIG_NMEA_STATEMENT_GSV)

    uint8_t cur_statement_gsv; /*!< Current satellite ID */
    uint8_t parsed_gsv;        /*!< OR'd of satellite system that have been parsed */
#endif

} esp_gps_t;

/**
 * @brief match satellite system from a message
 *              format of system in NMEA is $XX
 * @param esp_gps_t *esp_gps
 * @return SATELLITE_SYSTEM_UNKNOWN or SATELLITE_SYSTEM_XXX
 */
IRAM_ATTR static inline nmea_satellite_system_t match_satellite_system(esp_gps_t *esp_gps)
{
    static const char *const gnss_sat_system[CONFIG_NMEA_STATEMENT_GSX_DECODE_SYSTEM_NUM] = CONFIG_NMEA_STATEMENT_GSA_GSV_DECODE_SYSTEM_LIST();
    for (uint8_t i = 0; i < CONFIG_NMEA_STATEMENT_GSX_DECODE_SYSTEM_NUM; i++)
    {
        if (strstr(esp_gps->item_str, gnss_sat_system[i]))
        {
            ESP_LOGD("gps", "ss = %x", (i + 1));
            return (nmea_satellite_system_t)(i + 1);
        } // gnss_sat_system
    } // CONFIG_NMEA_STATEMENT_GSX_DECODE_SYSTEM_NUM
    ESP_LOGW("gps", "ss unknown");
    return SATELLITE_SYSTEM_UNKNOWN;
} // match_satellite_system
 ...
 */
IRAM_ATTR static float parse_lat_long(esp_gps_t *esp_gps)
{
 ...
 */
IRAM_ATTR static inline uint8_t convert_two_digit2number(const char *digit_char)
{
 ...
 */
IRAM_ATTR static void parse_utc_time(esp_gps_t *esp_gps)
{
 ...
 * @param esp_gps esp_gps_t type object
 */
IRAM_ATTR static void parse_gga(esp_gps_t *esp_gps)
{
    ...
    case 7: /* Satellites in use */
        ...
        break;
    case 8: /* HDOP */
        esp_gps->parent.dop_h_gga = strtof(esp_gps->item_str, NULL);
        break;
    case 9: /* Altitude */
 ...
 * @param esp_gps esp_gps_t type object
 */
IRAM_ATTR static void parse_gsa(esp_gps_t *esp_gps)
{
    /* Process GSA statement */
    switch (esp_gps->item_num)
    {
    case 1: /* msg type */
        ESP_LOGD("gps", "reset cnt");
        esp_gps->parent.sats_in_use_gsa[(esp_gps->prase_gnss_system - 1)] = 0;
        break;
    case 2: /* Process fix mode */
        esp_gps->parent.fix_mode[(esp_gps->prase_gnss_system - 1)] = (gps_fix_mode_t)strtol(esp_gps->item_str, NULL, 10);
        break;
    case 15: /* Process PDOP */
        esp_gps->parent.dop_p[(esp_gps->prase_gnss_system - 1)] = strtof(esp_gps->item_str, NULL);
        break;
    case 16: /* Process HDOP */
        esp_gps->parent.dop_h[(esp_gps->prase_gnss_system - 1)] = strtof(esp_gps->item_str, NULL);
        break;
    case 17: /* Process VDOP */
        esp_gps->parent.dop_v[(esp_gps->prase_gnss_system - 1)] = strtof(esp_gps->item_str, NULL);
        break;
    default:
        /* Parse satellite IDs */
        if (esp_gps->item_num >= 3 && esp_gps->item_num <= 14)
        {
            esp_gps->parent.sats_id_in_use[(esp_gps->prase_gnss_system - 1)][esp_gps->item_num - 3] = (uint8_t)strtol(esp_gps->item_str, NULL, 10);
            if (esp_gps->parent.sats_id_in_use[(esp_gps->prase_gnss_system - 1)][esp_gps->item_num - 3] != 0)
            {
                esp_gps->parent.sats_in_use_gsa[(esp_gps->prase_gnss_system - 1)]++;
                ESP_LOGD("gps", "cnt++");
            }
        }
        break;
 ...
 * @param esp_gps esp_gps_t type object
 */
IRAM_ATTR static void parse_gsv(esp_gps_t *esp_gps)
{
    ...
    case 1: /* total GSV numbers */
        esp_gps->sat_count[(esp_gps->prase_gnss_system - 1)] = (uint8_t)strtol(esp_gps->item_str, NULL, 10);
        break;
    case 2: /* Current GSV statement number */
        ESP_LOGD("gps", "pgsvsn , ip = %d ss = %d(start with 1) , str = %s", esp_gps->item_pos, esp_gps->prase_gnss_system, esp_gps->item_str);
        esp_gps->sat_num[(esp_gps->prase_gnss_system - 1)] = (uint8_t)strtol(esp_gps->item_str, NULL, 10);
        break;
    case 3: /* Process satellites in view */
        esp_gps->parent.sats_in_view[(esp_gps->prase_gnss_system - 1)] = (uint8_t)strtol(esp_gps->item_str, NULL, 10);
        break;
    default:
        if (esp_gps->item_num >= 4 && esp_gps->item_num <= 19)
        {
            ...
            uint8_t index;
            uint32_t value;
            index = 4 * (esp_gps->sat_num[(esp_gps->prase_gnss_system - 1)] - 1) + item_num / 4; /* Get array index */
            if (index < GPS_MAX_SATELLITES_IN_VIEW)
            {
                ...
                {
                case 0:
                    esp_gps->parent.sats_desc_in_view[(esp_gps->prase_gnss_system - 1)][index].num = (uint8_t)value;
                    break;
                case 1:
                    esp_gps->parent.sats_desc_in_view[(esp_gps->prase_gnss_system - 1)][index].elevation = (uint8_t)value;
                    break;
                case 2:
                    esp_gps->parent.sats_desc_in_view[(esp_gps->prase_gnss_system - 1)][index].azimuth = (uint16_t)value;
                    break;
                case 3:
                    esp_gps->parent.sats_desc_in_view[(esp_gps->prase_gnss_system - 1)][index].snr = (uint8_t)value;
                    break;
                default:
                    break;
                }
 ...
 * @param esp_gps esp_gps_t type object
 */
IRAM_ATTR static void parse_rmc(esp_gps_t *esp_gps)
{
 ...
 * @param esp_gps esp_gps_t type object
 */
IRAM_ATTR static void parse_gll(esp_gps_t *esp_gps)
{
 ...
 * @param esp_gps esp_gps_t type object
 */
IRAM_ATTR static void parse_vtg(esp_gps_t *esp_gps)
{
 ...
 */
IRAM_ATTR static esp_err_t parse_item(esp_gps_t *esp_gps)
{
    ...
    if (esp_gps->item_num == 0 && esp_gps->item_str[0] == '$')
    {
        ...
        else if (strstr(esp_gps->item_str, "GSA"))
        {
            ...
            // for gsa/gsv : match gnss system
            esp_gps->prase_gnss_system = match_satellite_system(esp_gps);
            esp_gps->cur_statement_gsa = esp_gps->prase_gnss_system;
        }
#endif
#if CONFIG_NMEA_STATEMENT_RMC
        ...
        else if (strstr(esp_gps->item_str, "GSV"))
        {
            ...
            // for gsa/gsv : match gnss system
            esp_gps->prase_gnss_system = match_satellite_system(esp_gps);
            esp_gps->cur_statement_gsv = esp_gps->prase_gnss_system;
        }
#endif
#if CONFIG_NMEA_STATEMENT_GLL
        ...
        goto out;
    } // esp_gps->item_num == 0 && esp_gps->item_str[0] == '$'
 ...
 * @return esp_err_t ESP_OK on success, ESP_FAIL on error
 */
IRAM_ATTR static esp_err_t gps_decode(esp_gps_t *esp_gps, size_t len)
{
    ...
    while (*d)
    {
        ...
        if (*d == '$')
        {
            ...
            for (uint8_t i = 0; i < CONFIG_NMEA_STATEMENT_GSX_DECODE_SYSTEM_NUM; i++)
            {
                esp_gps->sat_count[i] = 0;
                esp_gps->sat_num[i] = 0;
            }
#if CONFIG_NMEA_STATEMENT_GSA
            esp_gps->cur_statement_gsa = 0;
#endif
#if CONFIG_NMEA_STATEMENT_GSV
            esp_gps->cur_statement_gsv = 0;
#endif

            /* Add character to item */
            ESP_LOGD("gps", "gnssd , ip = %d d = %c , is = %s", esp_gps->item_pos, *d, esp_gps->item_str);
        ...
        {
            ESP_LOGD("gps", "gnssd call pi comm , ip = %d d = %c , is = %s , in = %d", esp_gps->item_pos, *d, esp_gps->item_str, esp_gps->item_num);
            /* Parse current item */
            parse_item(esp_gps);
        ...
        {
            ESP_LOGD("gps", "gnssd call pi star , ip = %d d = %c , is = %s , in = %d", esp_gps->item_pos, *d, esp_gps->item_str, esp_gps->item_num);
            /* Parse current item */
            parse_item(esp_gps);
            /* Asterisk detected */
            esp_gps->asterisk = 1;
        ...
        else if (*d == '\r')
        {
            ...
            if (esp_gps->crc == crc)
            {
                ...
                case STATEMENT_GSA:
                    ...
                    esp_gps->parsed_gsa |= 1 << esp_gps->cur_statement_gsa;
                    break;
#endif
#if CONFIG_NMEA_STATEMENT_RMC
                case STATEMENT_RMC:
                ...
                case STATEMENT_GSV:
                    uint8_t i;
                    for (i = 0; i < CONFIG_NMEA_STATEMENT_GSX_DECODE_SYSTEM_NUM; i++)
                    {
                        ESP_LOGD("gps", "i = %d  sn = %x  ,  sc = %x", i, esp_gps->sat_num[i], esp_gps->sat_count[i]);
                        if (esp_gps->sat_num[i] != esp_gps->sat_count[i])
                        {
                            break;
                        }
                    }
                    if (i == CONFIG_NMEA_STATEMENT_GSX_DECODE_SYSTEM_NUM)
                    {
                        esp_gps->parsed_statement |= 1 << STATEMENT_GSV;
                        esp_gps->parsed_gsv |= 1 << esp_gps->cur_statement_gsv;
                    }
...
#if CONFIG_NMEA_STATEMENT_GSA
                    esp_gps->parsed_gsa = 0;
#endif
#if CONFIG_NMEA_STATEMENT_GSV
                    esp_gps->parsed_gsv = 0;
#endif
 ...
 * @param esp_gps esp_gps_t type object
 */
IRAM_ATTR static void esp_handle_uart_pattern(esp_gps_t *esp_gps)
{
 ...
 * @param arg argument
 */
IRAM_ATTR static void nmea_parser_task_entry(void *arg)
{
...
nmea_parser_handle_t nmea_parser_init(const nmea_parser_config_t *config)
{
    ...
    esp_gps->buffer = calloc(1, config->uart_ring_buffer_size / 2);
    if (!esp_gps->buffer)
    {
...
#if (CONFIG_NMEA_STATEMENT_GSA || CONFIG_NMEA_STATEMENT_GSV)
    esp_gps->all_statements_gsx = 0;

    for (uint8_t i = 0; i < CONFIG_NMEA_STATEMENT_GSX_DECODE_SYSTEM_NUM; i++)
    {
        esp_gps->all_statements_gsx |= (1 << (i + 1));
    }
    ESP_LOGD("gps", "all_statements_gsx = %x", esp_gps->all_statements_gsx);
    ...
    // modify : put interrupt in iram
    if (uart_driver_install(esp_gps->uart_port, config->uart_ring_buffer_size, config->uart_tx_ring_buffer_size,
                            config->uart.event_queue_size, &esp_gps->event_queue, ESP_INTR_FLAG_IRAM) != ESP_OK)
    {
    ...
    BaseType_t err = xTaskCreate(
        ...
        "nmea_parser",
        config->parser_task_stack_size,
        esp_gps,
        config->parser_task_priority,
        &esp_gps->tsk_hdl);
    if (err != pdTRUE)
    {
    ...
    ESP_LOGD(GPS_TAG, "NMEA Parser init OK");
    return esp_gps;
    /*Error Handling*/
err_task_create:
